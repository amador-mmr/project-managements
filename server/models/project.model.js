...
Project.findAll = (result) => {
  ...
  p.KW, p.MONTHS_ACQUIRED, ca.ID_COMPANY
  FROM project p, company_adquires ca 
  WHERE ca.ID_PROJECT=p.ID_PROJECT`
...
Project.create = (project, result) => {
  ...
  (ID_PROJECT, NAME, CODE_3, ACQUISITION_DATE, DEAL_TYPE, ID_GROUP, STATUS, KW, MONTHS_ACQUIRED)
...
Project.edit = (id, project, result) => {
  ...
  SET NAME='${project.name}', CODE_3='${project.code_3}', ACQUISITION_DATE='${project.acquisition_date}', 
  DEAL_TYPE='${project.deal_type}', ID_GROUP='${project.deal_type}', STATUS='${project.status}', 
  KW=${project.kw}, MONTHS_ACQUIRED=${project.months_acquired} ID_PROJECT='${project.id_project}'
  WHERE ID_PROJECT='${id}';
  DELETE FROM test_project.wgt WHERE ID_PROJECT='${id}';
  `
